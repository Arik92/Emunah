app.service('hebService', function($http) {
  ...
  function getHolidays() {
    ...
    return $http.get('https://www.hebcal.com/hebcal/?v=1&cfg=json&maj=on&min=on&mod=on&nx=on&year=now&month='+currentMonth+'&mf=on&ss=on&c=off&s=off').then(function(result){
	  ...
	  if (nextHolidayIndex===-1) {		  
		  if (currentMonth===12) {
			  ...
			  currentMonth++;
			  return $http.get('https://www.hebcal.com/hebcal/?v=1&cfg=json&maj=on&min=on&mod=on&nx=on&year=now&month='+currentMonth+'&mf=on&ss=on&c=off&s=off').then(function(monthResult){
